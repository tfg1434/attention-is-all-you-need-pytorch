...
def main_wo_bpe():
    ...
    src_lang_model = spacy.load(spacy_model_map[opt.lang_src])
    trg_lang_model = spacy.load(spacy_model_map[opt.lang_trg])
    ...
    val = TranslationDataset(
        path=os.path.join(data_root, "validation"),
        exts=exts,
        fields=fields,
        filter_pred=filter_examples_with_length
    )
